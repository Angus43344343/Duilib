...
void CShadowUI::Create(CPaintManagerUI* pPaintManager)
{
	...
	CDuiString strKey;//zm
	strKey.Format(_T("%d"), (int)hParentWnd);//zm
	_ASSERT(GetShadowMap().Find(strKey) == NULL);//zm
	GetShadowMap().Insert(strKey, (LPVOID)this);//zm
...
CStdStringPtrMap& CShadowUI::GetShadowMap()
{
	static CStdStringPtrMap s_Shadowmap;
	return s_Shadowmap;
}
...
LRESULT CALLBACK CShadowUI::ParentProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	CDuiString strKey;//zm
	strKey.Format(_T("%d"), (int)hwnd);//zm
	_ASSERT(GetShadowMap().Find(strKey) != NULL);//zm
	CShadowUI* pThis = static_cast<CShadowUI*>(GetShadowMap().Find(strKey));//zm
...
#pragma warning(default: 4312)
	...
	case WM_WINDOWPOSCHANGED:
		...
		{
			//SetWindowPos(pThis->m_hWnd, 0, WndRect.left - pThis->m_rcShadowCorner.left, WndRect.top - pThis->m_rcShadowCorner.top, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);//zm
...
void CShadowUI::Update(HWND hParent)
{
	...
	if (m_bIsImageMode) 
	{
		...
		//nShadWndWid = WndRect.right - WndRect.left + m_rcShadowCorner.left + m_rcShadowCorner.right;//zm
		//nShadWndHei = WndRect.bottom - WndRect.top + m_rcShadowCorner.top + m_rcShadowCorner.bottom;//zm
...
void CShadowUI::MakeShadow(UINT32 *pShadBits, HWND hParent, RECT *rcParent)
{
	...
	// Get the region of parent window,
	HRGN hParentRgn = CreateRectRgn(0, 0, abs(rcParent->right - rcParent->left), abs(rcParent->bottom - rcParent->top));//zm
	...
	for(int i = 0; i <= 2 * nKernelSize; i++)
	{
		for(int j = 0; j <= 2 * nKernelSize; j++)
		{
			...
			else if(dLength <= nKernelSize)
			{
				...
				*pKernelIter = (UINT32)(nFactor << 24 | PreMultiply(m_Color, nFactor));
			}
			else
				*pKernelIter = 0;
...
void CShadowUI::ShowShadow(bool bShow)//显示阴影
{
...
bool CShadowUI::SetSharpness(unsigned int NewSharpness)//锐度
{
	if(NewSharpness > 35)
		return false;
...
bool CShadowUI::SetDarkness(unsigned int NewDarkness)//透明度
{
	if(NewDarkness > 255)
		return false;
...
bool CShadowUI::SetPosition(int NewXOffset, int NewYOffset)//偏移量
{
...
bool CShadowUI::SetColor(COLORREF NewColor)//颜色
{
	m_Color = NewColor;
	if(m_hWnd != NULL && (SS_VISABLE & m_Status))
		Update(GetParent(m_hWnd));
...
bool CShadowUI::SetImage(LPCTSTR szImage)//阴影图片
{
	if (szImage == NULL)
		return false;
...
bool CShadowUI::SetShadowCorner(RECT rcCorner)//九宫格模式加载图片
{
