...
namespace DuiLib
{
	...
	CDPI::CDPI()
	{
		...
		SetScale(MINITOR_DEFAULT_DPI);
	}

	int CDPI::GetDPIOfMonitor(HMONITOR hMonitor)
	{
		UINT dpix = MINITOR_DEFAULT_DPI, dpiy = MINITOR_DEFAULT_DPI;
		if (IsWindows8Point1OrGreater())//系统版本win8以上,否则不支持DWM虚拟机 
		{
			HRESULT  hr = E_FAIL;
	...
	{
		HMONITOR hMonitor;
		hMonitor = MonitorFromPoint(pt, MONITOR_DEFAULTTONEAREST);/*获取显示器句柄*/
		return GetDPIOfMonitor(hMonitor);/*获取显示器DPI*/
	}
	...
	UINT DuiLib::CDPI::GetDPI()
	{
		...
		{
			return MulDiv(m_nScaleFactorSDA, MINITOR_DEFAULT_DPI, 100);
		}

		return MulDiv(m_nScaleFactor, MINITOR_DEFAULT_DPI, 100);
	}
	...
	{
		m_nScaleFactor = MulDiv(uDPI, 100, MINITOR_DEFAULT_DPI);
