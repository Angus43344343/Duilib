...
namespace DuiLib
{
	IMPLEMENT_DUICONTROL(CHorizontalLayoutUI)
		CHorizontalLayoutUI::CHorizontalLayoutUI() : m_iSepWidth(0), m_uButtonState(0), m_bImmMode(false), m_dwSepColor(0xffFFFFFF)
	{
	...
	void CHorizontalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		rc.right -= m_rcInset.right + m_iSepWidth;//zm
	...
	bool CHorizontalLayoutUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
	{
	...
	void CHorizontalLayoutUI::SetSepColor(DWORD dwSepColor)
	{
		m_dwSepColor = dwSepColor;
	}

	DWORD CHorizontalLayoutUI::GetSepColor(DWORD dwSepColor)
	{
		return m_dwSepColor;
	}
