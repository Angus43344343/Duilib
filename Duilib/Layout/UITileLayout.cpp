...
namespace DuiLib
{
	...
	{
		if(m_pManager != NULL) return m_pManager->GetDPIObj()->Scale(m_szItem);//zm
		return m_szItem;
	}
	...
	void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		int cyNeeded = 0;
		int cxWidth = (rc.right - rc.left) / m_nColumns;//每个控件宽度
		if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) 
			cxWidth = (rc.right - rc.left + m_pHorizontalScrollBar->GetScrollRange() ) / m_nColumns; ;//加上滚动条位置
