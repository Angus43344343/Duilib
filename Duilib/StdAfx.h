...
#ifdef _DEBUG
#include <string.h>
#define MEMORY_ADDR_PATH "C:\\Users\\Administrator\\Desktop\\MemoryAttr.txt"
#define MEMORY_ADDR_READ(FileName, RowNum, pVoid) \
{\
	char acBuf[64] = { 0 }; \
	FILE* pFile = nullptr; \
	errno_t err = fopen_s(&pFile, MEMORY_ADDR_PATH, "ab"); \
if ((0 == err) && (nullptr != pFile)) \
	{\
	sprintf_s(acBuf, sizeof (acBuf), "%s.cpp     行:%d    addr:0x%p", FileName, RowNum, pVoid); \
	fwrite(acBuf, sizeof (acBuf), 1, pFile); \
	fwrite("\r\n", strlen("\r\n"), 1, pFile); \
	fclose(pFile); \
	}\
}
