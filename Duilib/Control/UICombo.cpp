...
namespace DuiLib {
	...
	void CComboWnd::Init(CComboUI* pOwner)
	{
		...
		for( int it = 0; it < pOwner->GetCount(); it++ ) 
		{
			...
			SIZE sz = pControl->EstimateSize(szAvailable);//计算每个控件的高度
			cyFixed += sz.cy;
		}
		cyFixed += 4;
		rc.bottom = rc.top + MIN(cyFixed, szDrop.cy);

		::MapWindowRect(pOwner->GetManager()->GetPaintWindow(), HWND_DESKTOP, &rc);//将窗口坐标转换成屏幕坐标

		MONITORINFO oMonitor = {};
		oMonitor.cbSize = sizeof(oMonitor);
		::GetMonitorInfo(::MonitorFromWindow(*this, MONITOR_DEFAULTTOPRIMARY), &oMonitor);//获取显示器信息
		CDuiRect rcWork = oMonitor.rcWork;
		if( rc.bottom > rcWork.bottom ) //当下拉窗口位于超过屏幕最底部位置时,下拉窗口显示在rcOwer的上方
		{
	...
	LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		if( uMsg == WM_CREATE ) 
		{
			...
			//m_pm.GetShadow()->ShowShadow(false);//zm
	
			m_pm.AttachDialog(m_pLayout);	
			m_pm.AddNotifier(this);

			//m_pm.GetShadow()->ShowShadow(m_pOwner->IsShowShadow());//zm
			//m_pm.GetShadow()->Create(&m_pm);//zm
		...
		else if( uMsg == WM_LBUTTONDOWN ) 
		{
			POINT pt = { 0 };
			::GetCursorPos(&pt);
			::ScreenToClient(m_pm.GetPaintWindow(), &pt);//需要转换
	...
	bool CComboUI::SelectItem(int iIndex, bool bTakeFocus)
	{
		...
		if( m_iCurSel >= 0 ) 
		{
			...
			if( pListItem != NULL ) pListItem->Select(false);//取消当前选中
			m_iCurSel = -1;
		}
