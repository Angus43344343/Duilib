...
#define DT_KEEP   3
	...
	RECT CDateTimeWnd::CalPos()
	{
		...
		{
			if( !pParent->IsVisible() )//上层容器不可见 
			{
				rcPos.left = rcPos.top = rcPos.right = rcPos.bottom = 0;
				break;
			}
			rcParent = pParent->GetClientPos();
			if( !::IntersectRect(&rcPos, &rcPos, &rcParent) ) //上层窗口与控件区域不相交
			{
				rcPos.left = rcPos.top = rcPos.right = rcPos.bottom = 0;
				break;
			}
	...
	void CDateTimeUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if(lstrcmpi(pstrName, _T("readonly")) == 0) SetReadOnly(lstrcmpi(pstrValue, _T("true")) == 0);
