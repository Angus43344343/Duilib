...
#define UILIST_MAX_COLUMNS 32
	...
	public:
		...
		virtual BOOL CheckColumCheckBoxable(int nColum) { return FALSE; };//zm

		virtual void DragBegin(TEventUI& event);//zm
		virtual void Draging(TEventUI& event);//zm
		virtual void DragEnd(TEventUI& event);//zm
	...
	public:
		...
		void SetShiftEnble(bool bShiftEnble);//zm
		bool GetShiftEnble();

		int HitTestColum(POINT ptMouse);//zm
	...
	public:
		...
		DWORD GetItemColor(int iIndex) const;//zm
		void SetItemColor(int iIndex, DWORD dwItemColor);//zm
		...
		virtual void DrawItemBk(HDC hDC, const RECT& rcItem);//zm
		virtual void DrawItemText(HDC hDC, const RECT& rcItem);
