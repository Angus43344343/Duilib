		...
		if( m_pManager != NULL ) 
		{
			if( !m_sGroupName.IsEmpty() ) 
			{
				if( m_bSelected ) 
				{
					...
					if(bMsg) 
					{
						m_pManager->SendNotify(this, DUI_MSGTYPE_SELECTCHANGED, 0, 0, true);//zm
					}
				}
			}
			else 
				...
				if(m_sSelectedImage.IsEmpty() && pImage != NULL)
				{
					...
					if( szImage.cx > 0 && szImage.cy > 0 )
					{
						...
						if(m_nSelectedStateCount > 1) 
						{
							...
							//m_sSelectedPushedImage.Format(_T("res='%s' restype='%s' dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"), info.sImageName.GetData(), info.sResType.GetData(), info.rcDest.left, info.rcDest.top, info.rcDest.right, info.rcDest.bottom, iLeft, iTop, iRight, iBottom);//zm
						}
