...
namespace DuiLib
{
	...
	void CGifAnimUI::InitGifImage()
	{
		...
		UINT nCount	= 0;
		nCount	=	m_pGifImage->GetFrameDimensionsCount();//获取帧维数对象:多页或多分辨率
		GUID* pDimensionIDs	=	new GUID[ nCount ];
		m_pGifImage->GetFrameDimensionsList( pDimensionIDs, nCount );//获取GUID列表
		m_nFrameCount	=	m_pGifImage->GetFrameCount( &pDimensionIDs[0] );//获取总帧数
		if (m_nFrameCount > 1)
		{
			int nSize = m_pGifImage->GetPropertyItemSize(PropertyTagFrameDelay);//获取特定属性项的大小
			m_pPropertyItem = (Gdiplus::PropertyItem*) malloc(nSize);
			m_pGifImage->GetPropertyItem(PropertyTagFrameDelay, nSize, m_pPropertyItem);//获取图像信息
		}
