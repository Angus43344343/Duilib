...
namespace DuiLib {
	...
	IMPLEMENT_DUICONTROL(CListUI)
		...
		CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_bMultiSel(false), m_iDragBeginIndex(-1)//zm
	{
	...
	bool CListUI::SelectMultiItem(int iIndex, bool bTakeFocus)
	{
		...
		//zm
		CListElementUI* pListElement = static_cast<CListElementUI*>(pControl->GetInterface(_T("ListElement")));
		if (pListElement == NULL) return false;
		if ((pListElement->GetShiftEnble()) && (GetSelectItemCount() > 0))
		{
	...
	void CListUI::DragBegin(TEventUI& event)
	{
		...
		if (::PtInRect(&rcListBody, event.ptMouse))
		{
			...
			if (NULL != pControl)
			{
				IListItemUI* pobjBeginItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
	...
	void CListUI::Draging(TEventUI& event)
	{
		...
		{		
			...
			if (::PtInRect(&rcListBody, event.ptMouse))
			{
				...
				if (NULL != pControlEnd)
				{
					IListItemUI* pobjEndItem = static_cast<IListItemUI*>(pControlEnd->GetInterface(_T("ListItem")));
					if (NULL != pobjEndItem)
					{
	...
	void CListHeaderItemUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			...
			else 
			{
				...
				m_pManager->SendNotify(this, DUI_MSGTYPE_LISTHEADERCLICK, 0, GetIndex());//zm
				Invalidate();
			}
			return;
		}
	...
	bool CListElementUI::Activate()
	{
		...
		if (m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMACTIVATE, GetIndex(), HitTestColum(m_pManager->GetMousePos()));//zm
		return true;
	}
	...
	bool CListElementUI::SelectMulti(bool bSelect)
	{
		...
		//if (m_pOwner != NULL) m_pOwner->SelectMultiItem(m_iIndex);//zm
		Invalidate();
		return true;
	}
	...
	int CListElementUI::HitTestColum(POINT ptMouse)
	{
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		for (int i = 0; i < pInfo->nColumns; i++)
		{
	...
	void CListLabelElementUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN )
		{
			if( IsEnabled() && (m_pOwner != NULL))
			{
				...
				else if ((GetKeyState(VK_SHIFT) & 0x8000))//zm
				{
					SetShiftEnble(true);
		...
		if( event.Type == UIEVENT_BUTTONUP ) 
		{
			if(IsEnabled() && (m_pOwner != NULL))
			{
				if (GetShiftEnble()) SetShiftEnble(false);//zm
				m_pOwner->DragEnd(event);//zm

				m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK, GetIndex(), HitTestColum(event.ptMouse));//zm
			}
			return;
		}
	...
	{
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		if (iIndex < 0 || iIndex >= pInfo->nColumns || m_aItemColors.GetSize() <= 0) return 0xff000000;

		DWORD dwColor = (DWORD)m_aItemColors.GetAt(iIndex);
		return dwColor;
	}
	...
	void CListTextElementUI::SetItemColor(int iIndex, DWORD dwItemColor)
	{
		...
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		if (iIndex < 0 || iIndex >= pInfo->nColumns) return;
		while (m_aItemColors.GetSize() < pInfo->nColumns) { m_aItemColors.Add((LPVOID)pInfo->dwTextColor); }
		m_aItemColors.SetAt(iIndex, (LPVOID)dwItemColor);
	...
	void CListTextElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
	{
		...
		for (int i = 0; i < pInfo->nColumns; i++)
		{
			RECT rcItem = { pInfo->rcColumn[i].left, m_rcItem.top, pInfo->rcColumn[i].right - 1, m_rcItem.bottom - 1 };
			//rcItem - 网格线的区域rc{1,1,1,1}
			DWORD iBackColor = GetItemColor(i);
			if (0xff000000 != iBackColor)
			{
	...
	int CListContainerElementUI::HitTestColum(POINT ptMouse)
	{
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		for (int i = 0; i < pInfo->nColumns; i++)
		{
