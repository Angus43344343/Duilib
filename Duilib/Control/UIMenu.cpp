...
namespace DuiLib {
	...
	void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem)
	{
		if (m_bShowExplandIcon)
		{
			...
			SIZE cxyFixed = GetManager()->GetDPIObj()->Scale(m_cxyFixed);//zm
	...
	void CMenuElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
	{
		...
		int nLinks = 0;
		RECT rcText = rcItem;
		RECT rcTextPadding = GetManager()->GetDPIObj()->Scale(pInfo->rcTextPadding);//zm
				...
				else
				{
					...
					if (IsWindow(GetManager()->GetPaintWindow()) && !isClosing) 
					{
						if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
						{
							...
							//zm
							SendMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)pMenuCmd, (LPARAM)this);
