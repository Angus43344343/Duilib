...
namespace DuiLib
{
	...
	SIZE CLabelUI::EstimateSize(SIZE szAvailable)//在布局容器被调用
	{
		//zm
		...
		if (m_bNeedEstimateSize) 
		{
			...
			m_cxyFixedLast = GetFixedSize();//zm
			// 自动计算宽度
			if ((m_uTextStyle & DT_SINGLELINE) != 0) 
			{
				...
				if (m_cxyFixedLast.cy == 0) 
				{
					...
					m_cxyFixedLast.cy += rcTextPadding.top + rcTextPadding.bottom;//zm
				}
				// 宽度
			...
			else if(m_cxyFixedLast.cy == 0) 
			{
				if(m_bAutoCalcHeight) 
				{
					...
					m_cxyFixedLast.cy = rcText.bottom - rcText.top + rcTextPadding.top + rcTextPadding.bottom;//zm
				}
			}
		}
	...
	void CLabelUI::SetText( LPCTSTR pstrText )
	{
		...
		if(GetAutoCalcWidth() || GetAutoCalcHeight()) //如果控件大小影响到容器的布局,就需要NeedParentUpdate();
		{
			NeedParentUpdate();
		}
	}
