...
err:
	...
	IMPLEMENT_DUICONTROL(CRichEditUI)
		...
		m_dwTipValueColor(0xFFBAC0C5), m_uTipValueAlign(DT_SINGLELINE | DT_LEFT), m_cPasswordChar('*'), m_bPassword(false)
	{
#ifndef _UNICODE
	...
	void CRichEditUI::SetPasswordChar(WCHAR cPasswordChar)
	{
	...
	void CRichEditUI::DoInit()
	{
		...
		if( m_pTwh ) 
		{
			if( m_bTransparent ) m_pTwh->SetTransparent(TRUE);//设置透明度
			if ( -1 != m_iFont ) SetFont(m_iFont);//zm 设置字体
			if (m_bPassword) m_pTwh->SetPasswordChar(m_cPasswordChar);//zm 设置密码字符
			...
			m_pManager->SetTimer(this, DEFAULT_TIMERID, ::GetCaretBlinkTime());//zm
			if (!m_bEnabled) 
			{
				m_pTwh->SetColor(m_pManager->GetDefaultDisabledColor());
			}
...
//zm 2020-06-19 让RichEdit继承CHorizontalLayoutUI,用来做简单控件嵌套
...
//zm 2020-06-19
	...
	void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		{
			if (_tcscmp(pstrValue, _T("true")) == 0) { m_lTwhStyle |= ES_PASSWORD; m_bPassword = true; }
		}
		else if (_tcscmp(pstrName, _T("passwordchar")) == 0)//zm
		{
