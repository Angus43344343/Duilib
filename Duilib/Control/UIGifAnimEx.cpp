...
namespace DuiLib
{
	...
	struct CGifAnimExUI::Imp
	{
		...
		virtual void LoadGifImage()
		{
			...
			drawInfo.Parse(m_pOwer->GetBkImage(), NULL, NULL);
			CDuiString sImageName = drawInfo.sImageName;
			//zm
			m_bLoadImg = true;
			m_pGifImage = CRenderEngine::LoadGifImageX(STRINGorID(sImageName), 0, 0);//zm

			if ( NULL == m_pGifImage ) return;
			m_nFrameCount	=	m_pGifImage->GetNumFrames();
			m_nFramePosition = 0;
			m_nDelay = m_pGifImage->GetFrameDelay() * 10;//zm
			if (m_nDelay <= 0 ) 
				m_nDelay = 100;
			if(!m_bAutoStart)
				m_bRealStop = true;
			...
			if (m_pOwer->IsAutoSize())//zm
			{
				m_pOwer->SetFixedWidth(m_pGifImage->GetWidth());
				m_pOwer->SetFixedHeight(m_pGifImage->GetHeight());
			}
		}
	...
	CGifAnimExUI::CGifAnimExUI(void) :m_pImp(new CGifAnimExUI::Imp(m_pManager)), m_bIsAutoSize(false)//zm
	{
		this;
		m_pImp->SetOwer(this);
	}
	...
	{
		return _T("GifAnimExUI");
	}

	LPVOID CGifAnimExUI::GetInterface( LPCTSTR pstrName )
	{
		if( _tcscmp(pstrName, DUI_CTR_GIFANIM_EX) == 0 ) return static_cast<CGifAnimExUI*>(this);
	...
	void CGifAnimExUI::SetAutoSize(bool bIsAuto)
	{
		m_bIsAutoSize = bIsAuto;
	}

	bool CGifAnimExUI::IsAutoSize() const
	{
