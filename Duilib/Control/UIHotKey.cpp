...
namespace DuiLib{
	...
	IMPLEMENT_DUICONTROL(CHotKeyUI)
		...
		CHotKeyUI::CHotKeyUI() : m_pWindow(NULL), m_wVirtualKeyCode(0), m_wModifiers(0), m_uButtonState(0), m_dwHotKeybkColor(0xFFFFFFFF), m_iHotKeyId(0)
	{
	...
	void CHotKeyUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_KILLFOCUS && IsEnabled() ) 
		{
			Invalidate();
			//zm
			if (0 != m_iHotKeyId) return;
			RegisterHotKey();//注册系统热键,产生WM_HOTKEY消息
		}
	...
	static UINT HotkeyToMod(UINT fsModifiers)
	{
