...
namespace DuiLib {
	...
	{
		CAnimationData* pData = GetAnimationDataByID(nAnimationID);//动画ID
		if( NULL != pData 
			|| nElapse <= 0
			|| nTotalFrame <= 0
			|| NULL == m_pControl )
	...
	void CUIAnimation::StopAnimation(int nAnimationID /*= 0*/)
	{
		...
		else
		{
			...
			for(int i=0; i<nCount; ++i)//移除所有定时器
			{
	...
	void CUIAnimation::OnAnimationElapse(int nAnimationID)
	{
		...
		if(nCurFrame == 0)
		{
			OnAnimationStart(nAnimationID, pData->m_bFirstLoop);//开始动画
			pData->m_bFirstLoop = FALSE;
		}

		OnAnimationStep(pData->m_nTotalFrame, nCurFrame, nAnimationID);//动画过程

		if(nCurFrame >= pData->m_nTotalFrame)//最后一帧
		{
			OnAnimationStop(nAnimationID);//关闭动画
			if(pData->m_bLoop)//是否循环
			{
				pData->m_nCurFrame = 0;
			}
			else
