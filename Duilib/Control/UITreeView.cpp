...
namespace DuiLib
{
	...
	void CTreeNodeUI::DoInit()
	{
		_ASSERT(NULL != GetTreeView());

		if (NULL != GetParentNode())//在这里设置首行缩进
		{
			pDottedLine->SetFixedWidth(GetParentNode()->GetDottedLine()->GetFixedWidth() + GetTreeView()->GetItemIndent());
			pItemButton->SetGroup(GetParentNode()->GetItemButton()->GetGroup());
		}
	}
	...
	bool CTreeNodeUI::AddAt( CControlUI* pControl, int iIndex )
	{
		...
		pControl = SetNodeAttr((CTreeNodeUI*)pControl);//zm
	...
	bool CTreeNodeUI::AddChildNode( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		_pTreeNodeUI = SetNodeAttr(_pTreeNodeUI);//zm
	...
	//	_pTreeNodeUI->GetDottedLine()->SetFixedWidth(pDottedLine->GetFixedWidth() + 16);//zm
	//	_pTreeNodeUI->SetParentNode(this);
	//	_pTreeNodeUI->GetItemButton()->SetGroup(pItemButton->GetGroup());//zm
	//	_pTreeNodeUI->SetTreeView(pTreeView);

	//	return _pTreeNodeUI;
	//}

	CTreeNodeUI* CTreeNodeUI::SetNodeAttr(CTreeNodeUI* _pTreeNodeUI)
	{
		_pTreeNodeUI->GetDottedLine()->SetVisible(TRUE);
		_pTreeNodeUI->SetParentNode(this);
		_pTreeNodeUI->SetTreeView(pTreeView);
	...
	// 函数说明: 
	//************************************
	CTreeViewUI::CTreeViewUI(void) : m_bVisibleFolderBtn(TRUE), m_bVisibleCheckBtn(FALSE), m_uItemMinWidth(0), m_uiIndent(0)//zm
	{
	...
	void CTreeViewUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		...
		else if (_tcsicmp(pstrName, _T("itemindent")) == 0)//zm
		{
	...
	void CTreeViewUI::SetItemIndent(UINT uiIndent)
	{
		m_uiIndent = uiIndent;
		Invalidate();
	}

	UINT CTreeViewUI::GetItemIndent()
	{
		return m_uiIndent;
	}
}