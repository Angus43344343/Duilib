...
namespace DuiLib {
	...
	CPaintManagerUI::~CPaintManagerUI()
	{
		...
		//清除资源
		RemoveAllFonts();
		//m_ResInfo.m_CustomFonts.Resize(0);

		RemoveAllImages();
		//m_ResInfo.m_ImageHash.Resize(0);

		RemoveAllStyle();
		//m_ResInfo.m_StyleHash.Resize(0);

		RemoveAllDefaultAttributeList();
		//m_ResInfo.m_AttrHash.Resize(0);

		RemoveAllDrawInfos();
		//m_ResInfo.m_DrawInfoHash.Resize(0);

		m_mNameHash.Resize(0);

		RemoveAllWindowCustomAttribute();
		//m_mWindowCustomAttrHash.Resize(0);
	...
	void CPaintManagerUI::SetLayered(bool bLayered)
	{
		if( m_hWndPaint != NULL && bLayered != m_bLayered ) 
		{
			...
			m_bLayered = bLayered;
			if( m_pRoot != NULL ) m_pRoot->NeedUpdate();//整个窗口更新
			Invalidate();
		}
	}
		...
		case WM_PAINT:
			{
				...
				{
					m_bUpdateNeeded = false;
					if( !::IsRectEmpty(&rcClient) && !::IsIconic(m_hWndPaint) )//该函数确定给定窗口是否是最小化（图标化）的窗口 
					{
						if( m_pRoot->IsUpdateNeeded() ) 
						{
							...
							m_pRoot->SetPos(rcRoot, true);//设置窗体内所有控件的位置
							bNeedSizeMsg = true;
						}
						else 
						{
							...
							for( int it = 0; it < m_aFoundControls.GetSize(); it++ )//查找需要更新的控件 
							{
								pControl = static_cast<CControlUI*>(m_aFoundControls[it]);
								if( !pControl->IsFloat() ) pControl->SetPos(pControl->GetPos(), true);
								else pControl->SetPos(pControl->GetRelativePos(), true);//相对位置
							}
							bNeedSizeMsg = true;
						}
				...
				else if( m_bLayered && m_bLayeredChanged )//分层窗口内的控件位置
				{
					RECT rcRoot = rcClient;
				...
				if( m_bLayered )//初始化分层窗口WS_EX_LAYERED
				{
				...
				if( m_bOffscreenPaint ) 
				{
					...
					//这里会进入CContainer的DoPaint：m_pRoot是XML文件里Window下的VerticalLayout或HorizontalLayout
					m_pRoot->Paint(m_hDcOffscreen, rcPaint, NULL);

					if( m_bLayered ) //处理nativaWindow
					{
					...
					if( m_bShowUpdateRect && !m_bLayered )//显示更新区域 
					{
	...
	void CPaintManagerUI::Term()
	{
		...
		m_SharedResInfo.m_ImageHash.Resize(0);
		...
		m_SharedResInfo.m_CustomFonts.Resize(0);
		...
		m_SharedResInfo.m_StyleHash.Resize(0);
		...
		m_SharedResInfo.m_AttrHash.Resize(0);

		//绘制信息
		CDuiString* pDrawInfo;
		for (int i = 0; i< m_SharedResInfo.m_DrawInfoHash.GetSize(); i++)
		{
			if (LPCTSTR key = m_SharedResInfo.m_DrawInfoHash.GetAt(i))
			{
				pDrawInfo = static_cast<CDuiString*>(m_SharedResInfo.m_DrawInfoHash.Find(key, false));
				if (pDrawInfo)
				{
					delete pDrawInfo;
					pDrawInfo = NULL;
		...
		m_SharedResInfo.m_DrawInfoHash.Resize(0);
		...
		CShadowUI::GetShadowMap().Resize(0);//zm 清理阴影窗口资源
	}
	...
	void DuiLib::CPaintManagerUI::SetDPI(int iDPI)
	{
		...
		if (!::IsZoomed(GetPaintWindow())) //窗口没有最大化
		{
			RECT rc = rcWnd;
...
//		m_bDragMode = false;
	...
	bool CPaintManagerUI::OnDrop(FORMATETC* pFmtEtc, STGMEDIUM& medium, DWORD *pdwEffect)//交给用户处理
	{
		POINT ptMouse = { 0 };
		...
		if (pFmtEtc->cfFormat == CF_HDROP && medium.tymed == TYMED_HGLOBAL)//拖放的文件名的结构体的句柄
		{
			HDROP hDrop = (HDROP)GlobalLock(medium.hGlobal);//锁定全局句柄指定的内存块
			if (hDrop != NULL)
			{
				::ScreenToClient(GetPaintWindow(), &ptMouse);
				CControlUI* pobjControl = static_cast<CControlUI*>(FindControl(ptMouse));
				if ((nullptr != pobjControl) && (pobjControl->IsDropEnabled()))
				{
					SendNotify(pobjControl, DUI_MSGTYPE_DROP, 0, (LPARAM)hDrop);//使用异步模式,防止死锁
				}
			}
			GlobalUnlock(medium.hGlobal);
		}
	...
	TFontInfo* CPaintManagerUI::GetDefaultFontInfo()
	{
		if (m_ResInfo.m_DefaultFontInfo.sFontName.IsEmpty())
		{
			if( m_SharedResInfo.m_DefaultFontInfo.tm.tmHeight == 0 ) 
			{
				...
				::GetTextMetrics(m_hDcPaint, &m_SharedResInfo.m_DefaultFontInfo.tm);//tm存放当前DC的字体信息
				::SelectObject(m_hDcPaint, hOldFont);
			}
	...
	void CPaintManagerUI::RemoveAllDrawInfos(bool bShared)
	{
		...
		if (bShared)
		{
			for (int i = 0; i< m_SharedResInfo.m_DrawInfoHash.GetSize(); i++)
			{
				if (LPCTSTR key = m_SharedResInfo.m_DrawInfoHash.GetAt(i))
				{
					pDrawInfo = static_cast<TDrawInfo*>(m_SharedResInfo.m_DrawInfoHash.Find(key, false));
					if (pDrawInfo)
					{
						delete pDrawInfo;
						pDrawInfo = NULL;
			...
			m_SharedResInfo.m_DrawInfoHash.RemoveAll();
		}
		else
		{
