...
namespace DuiLib {
	...
	CControlUI* CControlUI::GetParent() const //容器
	{
		return m_pParent;
	}
	...
	void CControlUI::SetDragEnable(bool bDrag)
	{
		...
		m_pManager->EnableDragDrop(m_bDragEnabled);//zm
	}
	...
	void CControlUI::SetDropEnable(bool bDrop)
	{
		...
		m_pManager->EnableDragDrop(m_bDropEnabled);//zm
	}
	...
	RECT CControlUI::GetRelativePos() const//相对位置
	{
		...
		if( pParent != NULL ) 
		{
			...
			rcRelativePos.Offset(-rcParentPos.left, -rcParentPos.top);//将子控件移到容器的原点位置
