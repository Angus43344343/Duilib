...
namespace DuiLib
{
	...
	void CContainerUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it = 0; it < m_items.GetSize(); it++ ) //容器内子控件位置
		{
			CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
			if( !pControl->IsVisible() ) continue;
			if( pControl->IsFloat() ) 
	...
	void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcsicmp(pstrName, _T("hscrollbarstyle")) == 0 ) 
		{
			...
			EnableScrollBar(TRUE, GetVerticalScrollBar() != NULL);
	...
	void CContainerUI::SetManager(CPaintManagerUI* pManager, CControlUI* pParent, bool bInit)
	{
		for( int it = 0; it < m_items.GetSize(); it++ ) 
		{
			static_cast<CControlUI*>(m_items[it])->SetManager(pManager, this, bInit);//所有子控件
		}
	...
	bool CContainerUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
	{
		...
		CRenderClip clip;
		CRenderClip::GenerateClip(hDC, rcTemp, clip);//给DC一个裁剪区域
		...
		if( m_items.GetSize() > 0 ) 
		{
			...
			if( !::IntersectRect(&rcTemp, &rcPaint, &rc) ) //指向两个源矩形相交的矩形
			{
				for( int it = 0; it < m_items.GetSize(); it++ ) 
				{
					CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
	...
	void CContainerUI::SetFloatPos(int iIndex)
	{
		...
		else 
		{
			...
			rcCtrl.left = (LONG)(width*rcPercent.left) + szXY.cx + left;//zm
			rcCtrl.top = (LONG)(height*rcPercent.top) + szXY.cy + top;//zm
			rcCtrl.right = (LONG)(width*rcPercent.right) + szXY.cx + sz.cx + left;//zm
			rcCtrl.bottom = (LONG)(height*rcPercent.bottom) + szXY.cy + sz.cy + top;//zm
			pControl->SetPos(rcCtrl, false);
		}
	}
