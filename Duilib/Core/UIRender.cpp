...
namespace DuiLib {
	...
	CRenderClip::~CRenderClip()
	{
		...
		::SelectClipRgn(hDC, hOldRgn);/*还原窗口裁剪区域*/
	...
	void CRenderClip::GenerateClip(HDC hDC, RECT rc, CRenderClip& clip)
	{
		...
		::ExtSelectClipRgn(hDC, clip.hRgn, RGN_AND);/*定义矩形裁剪区域*/
		clip.hDC = hDC;
		clip.rcItem = rc;
	}
	...
	void CRenderClip::GenerateRoundClip(HDC hDC, RECT rc, RECT rcItem, int width, int height, CRenderClip& clip)
	{
		...
		::ExtSelectClipRgn(hDC, clip.hRgn, RGN_AND);/*定义圆角矩形裁剪区域*/
		clip.hDC = hDC;
		clip.rcItem = rc;
	...
	static COLORREF PixelAlpha(COLORREF clrSrc, double src_darken, COLORREF clrDest, double dest_darken)/*颜色透明通道*/
	{
	...
	static BOOL WINAPI AlphaBitBlt(HDC hDC, int nDestX, int nDestY, int dwWidth, int dwHeight, HDC hSrcDC, \
		int nSrcX, int nSrcY, int wSrc, int hSrc, BLENDFUNCTION ftn)/*位图透明通道,使用DIB:与设备无关*/
	{
		...
		do 
		{
			if( type == NULL )
			{
				...
				else 
				{
...
#ifdef UNICODE
						char* pwd = w2a((wchar_t*)sFilePwd.GetData());
						hz = OpenZip((void*)sFile.GetData(), sFile.GetLength(), pwd);//zm
						if(pwd) delete[] pwd;
#else
					...
					if( dwSize == 0 ) break;
					pData = new BYTE[ dwSize ];
					//int res = UnzipItem(hz, i, pData, dwSize, 3);//zm
					int res = UnzipItem(hz, i, pData, dwSize);//zm
					if( res != 0x00000000 && res != 0x00000600)
					{
						delete[] pData;
						pData = NULL;
	...
	{
#ifdef USE_GDI_RENDER /*DGI渲染，默认使用的GDI+*/
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	...
	void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
	{
		...
		if ( pManager->IsLayered() || pManager->IsUseGdiplusText())
		{
			...
			{
			case 0: {trh = Gdiplus::TextRenderingHintSystemDefault; break;}//使用与系统相同的处理方式
			case 1: {trh = Gdiplus::TextRenderingHintSingleBitPerPixelGridFit; break;}//不消除锯齿,使用网格匹配
			case 2: {trh = Gdiplus::TextRenderingHintSingleBitPerPixel; break;}//不消除锯齿,不使用网格匹配
			case 3: {trh = Gdiplus::TextRenderingHintAntiAliasGridFit; break;}//消除锯齿,使用网格匹配
			case 4: {trh = Gdiplus::TextRenderingHintAntiAlias; break;}//消除锯齿,不使用网格匹配
			case 5: {trh = Gdiplus::TextRenderingHintClearTypeGridFit; break;}//在液晶显示器上使用ClearType技术增强字体清晰度
			}
	...
	HBITMAP CRenderEngine::GenerateBitmap(CPaintManagerUI* pManager, CControlUI* pControl, RECT rc, DWORD dwFilterColor)
	{
		...
		{
			HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(hCloneDC, hBitmap);
			::BitBlt(hCloneDC, 0, 0, cx, cy, hPaintDC, rc.left, rc.top, SRCCOPY);//拷贝
			RECT rcClone = {0, 0, cx, cy};
			if (dwFilterColor > 0x00FFFFFF) DrawColor(hCloneDC, rcClone, dwFilterColor);
			::SelectObject(hCloneDC, hOldBitmap);
			::DeleteDC(hCloneDC);  
