...
HWND CWindowWnd::Subclass(HWND hWnd)
{
    ...
    m_bSubclassed = true;
    m_hWnd = hWnd;
    ::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(this));//设置对象私有数据
    return m_hWnd;
}
...
UINT CWindowWnd::ShowModal()
{
    ASSERT(::IsWindow(m_hWnd));
    UINT nRet = 0;
	//如果主窗口,则没有ower窗口；如果是弹出窗口则ower窗口为主窗口
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) //屏蔽标题栏
	{
        LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
        pThis = static_cast<CWindowWnd*>(lpcs->lpCreateParams);
        pThis->m_hWnd = hWnd;
        ::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(pThis));//对象私有数据.pThis
    } 
    else 
	{
        pThis = reinterpret_cast<CWindowWnd*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));//获取,与上面对应
        if( uMsg == WM_NCDESTROY && pThis != NULL )//窗口销毁时处理 
		{
